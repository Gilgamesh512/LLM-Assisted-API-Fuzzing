...
import { EXTRA_MODEL_PROVIDERS, type ModelConfigSelection, type ReasoningEffort } from "@vegamo/deepcode-core";
...
// DeepSeek models first (native to this fork), then any extra providers
// (e.g. Gemini via OpenAI-compat endpoint — see EXTRA_MODEL_PROVIDERS in
// model-capabilities.ts). Selecting an extra-provider model automatically
// swaps env.BASE_URL/env.API_KEY — see applyModelConfigSelection in
// packages/core/src/settings.ts.
export const MODEL_COMMAND_MODELS = [
  "deepseek-v4-pro",
  "deepseek-v4-flash",
  ...EXTRA_MODEL_PROVIDERS.map((provider) => provider.model),
] as const;

function describeModel(model: string): string {
  return EXTRA_MODEL_PROVIDERS.find((p) => p.model === model)?.label ?? "";
}
      ...
      ? MODEL_COMMAND_MODELS.map((model) => ({
          ...
          description:
            model === modelConfig.model
              ? ["current model", describeModel(model)].filter(Boolean).join(" — ")
              : describeModel(model),
