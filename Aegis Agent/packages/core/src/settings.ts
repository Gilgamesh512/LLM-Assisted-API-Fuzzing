...
export function applyModelConfigSelection(
  ...
  if (selected.model !== current.model) {
    next.env = applyProviderEnvForModelSwitch(next.env, selected.model);
  }
...
/**
 * Adjusts `env.BASE_URL`/`env.API_KEY` when the newly-selected model belongs
 * to a different provider than the built-in DeepSeek endpoint (see
 * `EXTRA_MODEL_PROVIDERS` in model-capabilities.ts).
 *
 * Switching FROM DeepSeek TO an extra provider (e.g. Gemini): points
 * `BASE_URL` at that provider and, if a key was previously stashed under its
 * dedicated `env.<PROVIDER>_API_KEY` field, copies it into the active
 * `env.API_KEY`.
 *
 * Switching FROM an extra provider back TO a DeepSeek model: removes the
 * override `BASE_URL` (falls back to the DeepSeek default) and restores the
 * DeepSeek key from `env.DEEPSEEK_API_KEY` if one was stashed.
 *
 * Switching between two DeepSeek models never reaches either branch (neither
 * is an "extra provider"), so `env` is returned completely untouched —
 * existing custom `BASE_URL` overrides (e.g. a Coding Plan endpoint) survive.
 */
function applyProviderEnvForModelSwitch(
  env: DeepcodingSettings["env"],
  selectedModel: string
): DeepcodingSettings["env"] {
  const newProvider = findExtraModelProvider(selectedModel);
  const nextEnv = { ...(env ?? {}) };

  if (newProvider) {
    nextEnv.BASE_URL = newProvider.baseURL;
    const stashedKey = nextEnv[newProvider.apiKeyEnv];
    if (typeof stashedKey === "string" && stashedKey.trim()) {
      nextEnv.API_KEY = stashedKey;
    }
    return nextEnv;
  }

  const wasExtraProvider = env?.BASE_URL && EXTRA_MODEL_PROVIDERS.some((p) => p.baseURL === env.BASE_URL);
  if (wasExtraProvider) {
    delete nextEnv.BASE_URL;
    const stashedKey = nextEnv["DEEPSEEK_API_KEY"];
    if (typeof stashedKey === "string" && stashedKey.trim()) {
      nextEnv.API_KEY = stashedKey;
    }
  }
