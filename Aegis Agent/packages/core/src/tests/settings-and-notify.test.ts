...
test("applyModelConfigSelection switches BASE_URL/API_KEY when selecting an extra-provider model (e.g. Gemini)", () => {
  const result = applyModelConfigSelection(
    {
      env: {
        MODEL: "deepseek-v4-pro",
  ...
  assert.equal(result.changed, true);
  assert.equal(result.settings.model, "gemini-2.5-flash");
  assert.equal(result.settings.env?.BASE_URL, "https://generativelanguage.googleapis.com/v1beta/openai/");
  assert.equal(result.settings.env?.API_KEY, "gemini-key-test");
  // Key khác vẫn còn nguyên, để đổi ngược lại sau này không mất key DeepSeek.
  assert.equal(result.settings.env?.DEEPSEEK_API_KEY, "sk-deepseek-test");
});

test("applyModelConfigSelection restores the DeepSeek default BASE_URL/API_KEY when switching back from an extra provider", () => {
  const result = applyModelConfigSelection(
    {
      env: {
        MODEL: "gemini-2.5-flash",
  ...
  assert.equal(result.changed, true);
  assert.equal(result.settings.model, "deepseek-v4-pro");
  assert.equal(result.settings.env?.BASE_URL, undefined); // falls back to DEFAULT_BASE_URL
  assert.equal(result.settings.env?.API_KEY, "sk-deepseek-test");
});

test("applyModelConfigSelection leaves env completely untouched when switching between two DeepSeek models", () => {
  // Guards against regressing a custom BASE_URL (e.g. a Coding Plan endpoint)
  // used together with a deepseek-* model name.
  const result = applyModelConfigSelection(
    {
      env: {
        MODEL: "deepseek-v4-pro",
        BASE_URL: "https://ark.cn-beijing.volces.com/api/coding/v3",
        API_KEY: "custom-plan-key",
      },
      thinkingEnabled: true,
    },
    {
  ...
  assert.equal(result.changed, true);
  assert.equal(result.settings.env?.BASE_URL, "https://ark.cn-beijing.volces.com/api/coding/v3");
  assert.equal(result.settings.env?.API_KEY, "custom-plan-key");
});
